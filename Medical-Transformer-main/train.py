...
for epoch in range(args.epochs):
    ...
    # ===================log========================
    print('epoch [{}/{}], loss:{:.4f}'
          .format(epoch, args.epochs, avg_loss))
    ...
    if (epoch % args.save_freq) == 0:
        ...
        for batch_idx, (X_batch, y_batch, *rest) in enumerate(valloader):
            ...
            # 计算变化率（作为示例指标）
            variation_rate = np.mean(np.abs(yHaT - yval))

            # 记录验证指标到 wandb
            wandb.log({'variation_rate': variation_rate})
            ...
            # 检查数组形状
            print("X_batch shape:", X_batch.cpu().numpy().shape)
            print("yHaT shape:", yHaT.shape)
            print("yval shape:", yval.shape)

            # 根据数组形状调整索引
            ground_truth_image = yval[0, :, :]
            prediction_image = yHaT[0, 0, :, :]

            # 记录图像到 wandb
            wandb.log({
                'Input Image': wandb.Image(X_batch.cpu()[0]),
                'Prediction': wandb.Image(prediction_image),
                'Ground Truth': wandb.Image(ground_truth_image),
            })
